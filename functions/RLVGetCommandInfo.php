...
function RLVGetCommandInfo($command) 
{
    
    // Environement variables for database connection
    global $config;

    // Get database connection details from the global config
    ...
    if ($conn->connect_error) {
        error_log("RLVGetInfo: Database connection failed: " . $conn->connect_error);
        return json_encode(null);
    }

    // Prepare and execute the stored procedure
    $stmt = $conn->prepare("CALL find_matching(?)");
    if (!$stmt) {
        error_log("RLVGetInfo: Statement preparation failed: " . $conn->error);
        $conn->close();
        return json_encode(null);
    }
    $stmt->bind_param("s", $command);
    if (!$stmt->execute()) {
        error_log("RLVGetInfo: Execution failed: " . $stmt->error);
        $stmt->close();
        $conn->close();
        return json_encode(null);
    }
    ...
    // Cleanup: close statement and connection
    $stmt->close();
    $conn->close();

    // Return JSON string (null if no results)
